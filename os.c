...
// **similar to Lab 3. initialize priority field****
  ...
  tcbs[0].next = &tcbs[1];
	...
	tcbs[6].next = &tcbs[7];
	tcbs[7].next = &tcbs[0];
	// initialize to not blocked and not sleeping
...
// In Lab 4, handle periodic events in RealTimeEvents
  uint32_t i;
	for (i=0; i<NUMTHREADS; i++){//if thread is sleeping decrement sleep time
	  if (tcbs[i].sleeping){  
	    (tcbs[i].sleeping)--;
		}
	}
...
void OS_Launch(uint32_t theTimeSlice){
  ...
  STCTRL = 0x00000007;         // enable, core clock and interrupt arm
	BSP_PeriodicTask_InitB(&runperiodicevents, 1000, 6); // init timer to decrement tcbType->Sleep
...
// If there are multiple highest priority (not blocked, not sleeping) run these round robin
	...
	do{
		pt = pt->next;
		if ((pt->blocked) == 0 && (pt->sleeping) == 0 && (pt->priority) < maxPri){
		  bestPt = pt;
			maxPri = pt->priority;
		}
  } while (pt != RunPt);
...
// Same as Lab 3
	tcbType *pt;
	DisableInterrupts();
	(*semaPt)++; // increment semaphore
	
	if ((*semaPt) <= 0){ // unblock one thread (next blocked in the list)
		pt = RunPt->next;
...
//***IMPLEMENT THIS***
	...
	SYSCTL_RCGCGPIO_R |= 0x08;      // (a) activate clock for Port D 
	...
	GPIO_PORTD_PUR_R &= ~0x40;      // disable pullup resistor 
